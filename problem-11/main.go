	...
	for r := 0; r < len(matrix); r++ {
		for c := 0; c < len(matrix[r]); c++ {
			if r >= 3 && c < len(matrix[r])-3 {
				// diagonally up
				m := matrix[r][c] * matrix[r-1][c+1] * matrix[r-2][c+2] * matrix[r-3][c+3]
				if max < m {
					max = m
				}
			}
			if c < len(matrix[r])-3 {
				// right
				m := matrix[r][c] * matrix[r][c+1] * matrix[r][c+2] * matrix[r][c+3]
				if max < m {
					max = m
				}
			}
			if r < len(matrix)-3 {
				...
				// down
				m := matrix[r][c] * matrix[r+1][c] * matrix[r+2][c] * matrix[r+3][c]
				if max < m {
					max = m
				}
			}
